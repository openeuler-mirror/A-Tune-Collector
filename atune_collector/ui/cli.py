...
class DisplayScreen:
    ...
    def __init__(self):
        ...
        curses.init_pair(COLOR_BLUE_BLACK, curses.COLOR_BLUE, curses.COLOR_BLACK)
    ...
    def win_init(self):
        ...
        if self.height < 50 or self.width < 200:
            ...
            self.screen.addstr(self.height//2+1, abs(self.width-len(notemsg))//2, notemsg, curses.A_REVERSE)
            while self.key != KEY_EXIT:
                self.screen.refresh()
                self.height, self.width = self.screen.getmaxyx()
                if self.height >= 50 and self.width >= 200:
                    self.errstatus = False
                    break
    ...
    def draw_progressbar(window, name:str, begin_y=1, begin_x=1,
        ...
        if len(info.keys()) > 7:
            raise Exception("Catch Too Much Sorts of Data to Draw Prgress Bar! (>7)")
    ...
    def display_welcome(self):
        ...
        win_welcome = curses.newwin(10, 100, (self.height-10)//2, (self.width-100)//2)
        win_welcome.addstr(0, 0, logo, curses.color_pair(COLOR_RED_BLACK) | curses.A_BOLD)
        win_welcome.addstr(6, 0, info, curses.color_pair(COLOR_BLUE_BLACK) | curses.A_BOLD)
    ...
    def display_mem(self, avg_data={},
        ...
        for key, val in rdata.items():
            if val in avg_data:
                rdata[key] = round((avg_data[val] / 1024) if key != "util" else avg_data[val] , 1)
    ...
    def display_network(self, dev=None, avg_data={},
        ...
        if len(dev) == 1:
            ...
            self.draw_diagram(win_nic1, item[0], "Time", 10, 20, 2, 35, plot_data[item[0]])
            self.draw_diagram(win_nic1, item[1], "Time", 10, 20, 2, 65, plot_data[item[1]])
        elif len(dev) >= 2:
            win_nic1 = self.win_network.derwin(height-2, width//2-1, 1, 1)
            win_nic2 = self.win_network.derwin(height-2, width//2-1, 1, width//2)
            ...
            self.draw_diagram(win_nic1, item[0], "Time", 10, 18, 2, 24, plot_data[item[0]])
            self.draw_diagram(win_nic2, item[1], "Time", 10, 18, 2, 24, plot_data[item[1]])
        self.win_network.refresh()
    ...
    def display_perf(self, avg_data={},
        ...
        self.draw_multiline(self.win_perf, 3, 2, curses.A_BOLD,
                               contents=["IPC: {}".format(rdata["ipc"]),
                                         ...
                                         "ITLBLMR: {} %".format(rdata["itlbmr"]),
                                         "DTLBLMR: {} %".format(rdata["dtlbmr"]),
                                         "SBPI: {}".format(rdata["sbpi"]),
                                         "SBPC: {}".format(rdata["sbpc"])])
        
        self.draw_diagram(self.win_perf, item[0], "Time", 8, 30, 2, 20, plot_data[item[0]])
        self.draw_diagram(self.win_perf, item[1], "Time", 8, 30, 2, 60, plot_data[item[1]])
        self.win_perf.refresh()
    ...
    def display_system(self, avg_data={},
        ...
        self.draw_multiline(self.win_system, 2, 2, curses.A_BOLD,
                            contents=[
                                "New Process: {} proc/s".format(rdata["procs"]),
                                "Context Switch: {} sw/s".format(rdata["cswchs"])
                            ])
        self.win_system.hline(4, 2, curses.ACS_HLINE, 30)
        self.draw_multiline(self.win_system, 5, 2, curses.A_BOLD,
                            contents=[
                                "Running Queue Size: {}".format(rdata["runqsz"]),
                                "Process List Size: {}".format(rdata["plistsz"]),
                                "Last 1-min Load Average: {}".format(rdata["ldavg1"]),
                                "Last 5-min Load Average: {}".format(rdata["ldavg5"])
                            ])
        ...
        item = list(plot_data.keys())
        self.draw_diagram(self.win_system, item[0], "Time", 8, 20, 2, 35, plot_data[item[0]])
        self.draw_diagram(self.win_system, item[1], "Time", 8, 20, 2, 65, plot_data[item[1]])
        self.win_system.refresh()
